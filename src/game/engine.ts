...
export function createState(
  ...
  const def = CLASSES[cls] ?? CLASSES.grunt;
  warmAccessories(def.accessories);
  // the hero you picked always stays the body; the class only adds the headgear
...
export function render(ctx: CanvasRenderingContext2D, s: GameState, sprites: Sprites, time: number) {
  ...
  const drawEcho = (e: Echo) => {
    ...
    const strips =
      sprites.playerSkins[e.character] ??
      sprites.playerSkins.spike;
    const anim: AnimKey = e.moving ? "walk" : "idle";
    ...
    {
      const wear = CLASSES[s.class]?.accessories ?? [];
      if (wear.length && strip.img.width) {
        drawWorn(ctx, wear, {
          skin: e.character,
          anim,
          frame,
          aspect: strip.img.width / strip.frames / strip.img.height,
          x: e.x + glitch,
          y: e.y,
          height: eh,
          flip: e.facing === -1,
  ...
  const pSkin =
    sprites.playerSkins[p.character] ??
    sprites.playerSkins.spike;
  const pStrip = pSkin[pAnim];
  ...
  {
    const wear = CLASSES[p.class ?? s.class]?.accessories ?? [];
    const img = pStrip.img;
    if (wear.length && img.width) {
      drawWorn(ctx, wear, {
        skin: p.character,
        anim: pAnim,
        frame: pFrame,
        aspect: img.width / pStrip.frames / img.height,
        x: p.x,
        y: p.y,
        height: ph,
        flip: p.facing === -1,
